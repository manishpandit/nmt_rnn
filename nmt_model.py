...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        # Construct Tensor `X`
        src_word_embeds = self.model_embeddings.source(source_padded)
        X = pack_padded_sequence(src_word_embeds, source_lengths)
        
        # Compute `enc_hiddens`, `last_hidden`, `last_cell` 
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in Y.split(split_size=1):
            ...
            (h_t, cell_t), o_t, e_t = self.step(Ybar_t, dec_state,
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_view = (e_t.size(0), 1, e_t.size(1))
        a_t = torch.bmm(alpha_t.view(*a_view), enc_hiddens).squeeze(1)
